...
object SparkExample {
  ...
  val kuduMasters: String = System.getProperty("kuduMasters", "server1:7051,server2:7051,server3:7051")
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder.appName("KuduSparkExample").master("local[1]").getOrCreate()
    ...
    val schema = StructType(
      List(
        ...
        StructField(nameCol, StringType, nullable = false)
      )
    )
    ...
    try {
      ...
      val sqlDF = spark.sqlContext.read.options(
        Map("kudu.master" -> kuduMasters, "kudu.table" -> tableName)).format("kudu").load
